...
- **Integration** — the `/summarise` endpoint end-to-end via `TestClient`: the happy path
  ...
  production codebase against a fixed spec, tests would be included within each feature PR - which the CI workflow below
  would enforce (once branch protection is enabled).

## Continuous integration

A GitHub Actions workflow (`.github/workflows/ci.yml`) runs on every pull request and on
pushes to `main`:

- `ruff check` — linting
- `ruff format --check` — formatting is verified (not modified)
- `pytest` — the full test suite

This surfaces a pass/fail status on every pull request. Enabling branch protection on
`main` (requiring this check to pass) would then enforce it as a merge gate.
